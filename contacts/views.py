...
def inquiry(request):
    if request.method == 'POST':
        ...
        # checks if the user has previous inquiry on the same car, then we dont save
        if request.user.is_authenticated:
            user_id = request.user.id
            has_contacted = Contact.objects.all().filter(car_id=car_id, user_id=user_id)
            if has_contacted:
                messages.error(
                    request, 'You have alreay made an inquiry about this car. Please wait until we get back to you.')
        ...
        send_mail(
            "New Car Inquiry",
            "You have a new inqury for the car " + car_title +
            '. Please login to your admin panel for more details. ',
            "rosy61625@gmail.com",
            [admin_email],
            fail_silently=False,
        )
